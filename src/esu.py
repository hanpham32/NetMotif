...
class ESU:
    def __init__(self, G: nx.Graph, size: int, graph_type: GraphType):
        ...
        for i, node in enumerate(self.nodes):
            ...
            self.esu_recursive_helper(self.size, set(self.get_right_neighbors(node)), node_list, self.subgraph_list, self.node_visited)
        ...
        for i, subgraph in enumerate(self.subgraph_list):
            ...
            if d6 not in self.label_conversion_map:
                ...
                g6 = lb.get_graph_label(sub.G, self.graph_type)  # Expensive operation, minimize calls!
